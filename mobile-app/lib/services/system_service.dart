...
/// §8 - Interface technique : suivi des API (gateway/circuit breaker) et
/// gestion des préfixes réseau.
class SystemService {
  SystemService._();
  ...
  /// Interroge /__gateway/status directement (hors /api/v1, pas via
  /// ApiClient) avec la clé de debug partagée.
  Future<List<ServiceStatus>> fetchGatewayStatus() async {
    final uri = Uri.parse('${ApiConfig.gatewayRootUrl}/__gateway/status');
    final response = await _http.get(uri, headers: {
      'X-Fripay-Debug-Key': ApiConfig.gatewayDebugKey,
    }).timeout(_timeout);

    if (response.statusCode != 200) {
      throw Exception('Statut gateway indisponible (${response.statusCode})');
    }

    final decoded = jsonDecode(response.body) as Map<String, dynamic>;
    final services = (decoded['services'] as Map<String, dynamic>? ?? {});
    return services.entries
        .map((e) => ServiceStatus.fromEntry(e.key, e.value as Map<String, dynamic>))
        .toList();
  }
